reset

set terminal postscript eps enhanced color"Times-Roman" 18
set autoscale
set lmargin 13
set bmargin 4

set output "residuals.eps"

set ylabel"{/*1.5{/Italic residuo inicial}" offset 1,0
set xlabel"{/*1.5{/Italic tempo (s)} }" offset 0,0

set logscale y
set format y "%.1e"

plot "logs/Ux_0"  using ($1):($2) title"{U_x}" with lines lt 3 lw 2.5, \
     "logs/Uy_0"  using ($1):($2) title"{U_y}" with lines lt 2 lw 2.5, \
     "logs/p_1"  using ($1):($2) title"{p}" with lines lt 1 lw 2.5, \
     "logs/k_0"  using ($1):($2) title"{k}" with lines lt 4 lw 2.5, \
     "logs/epsilon_0"  using ($1):($2) title"{epsilon}" with lines lt 5 lw 2.5



